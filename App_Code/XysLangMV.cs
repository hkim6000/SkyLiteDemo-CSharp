...
public class XysLangMV : WebBase
{
    ...
    {
        return new ViewPart { Methods = ViewMethods("XysLang") };
    }
    ...
    private void SetGridStyle(SQLGridSection SQLGrid)
    {
        ...
        {
            SQLGrid.Grid.TableColumns[1].SetColumnAttribute(HtmlEvents.onclick, ByPassCall("MenuClick", "m=" + References.Pages.XysLangEV + "&t={0}{1}"));
    ...
    public ApiResponse SearchClicked()
    {
        ...
        SQLGridInfo.Query.Filters = "CODE=N'ISO639' and CODE+SD01+SD02+SD03+SD04+SD05+SD06+SD07  like N'%" + sbox + "%' ";
        SQLGridSection SQLGrid = new SQLGridSection(SQLGridInfo);
        SetGridStyle(SQLGrid);
